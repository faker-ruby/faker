...
class TestPlaceholdit < Test::Unit::TestCase
  ...
  def test_avatar_with_custom_size
    assert @tester.image(size: '3x3').match(/https:\/\/placehold\.it\/+(\d+x\d+)/)[1] == '3x3'
  end
  ...
  def test_avatar_with_supported_format
    assert @tester.image(size: '300x300', format: 'jpg').match(/https:\/\/placehold\.it\/(.+)(jpg?)/)
  end
  ...
  def test_avatar_background_with_correct_six_char_hex
    assert @tester.image(size: '300x300', format: 'jpg', background_color: 'ffffff').match(/https:\/\/placehold\.it\/(.+)(jpg?)\/ffffff/)
  end

  def test_avatar_background_with_correct_three_char_hex
    assert @tester.image(size: '300x300', format: 'jpg', background_color: 'fff').match(/https:\/\/placehold\.it\/(.+)(jpg?)\/fff/)
  end
  ...
  def test_avatar_font_color_with_correct_six_char_hex
    assert @tester.image(size: '300x300', format: 'jpg', background_color: 'ffffff', text_color: '000000').match(/https:\/\/placehold\.it\/(.+)(jpg?)\/ffffff\/000000/)
  end

  def test_avatar_font_color_with_correct_three_char_hex
    assert @tester.image(size: '300x300', format: 'jpg', background_color: 'fff', text_color: '000').match(/https:\/\/placehold\.it\/(.+)(jpg?)\/fff/)
  end
  ...
  def test_text_not_present
    assert @tester.image(size: '300x300', format: 'jpg', background_color: 'fff', text_color: '000').match(/https:\/\/placehold\.it\/[^\\?]+$/)
  end

  def test_text_present
    assert @tester.image(size: '300x300', format: 'jpg', background_color: 'fff', text_color: '000', text: 'hello').match(/https:\/\/placehold\.it\/(.+)\?text=hello/)
  end
