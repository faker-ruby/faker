...
class TestDeterminism < Test::Unit::TestCase
  ...
  def subclasses
    Faker.constants.delete_if do |subclass|
      %i[Base Bank Char Base58 ChileRut Config Creature Date DragonBall Dota ElderScrolls Fallout Games GamesHalfLife HeroesOfTheStorm Internet JapaneseMedia LeagueOfLegends Movies Myst Overwatch Pokemon OnePiece SwordArtOnline Time VERSION Witcher WorldOfWarcraft Zelda].include?(subclass)
    end.sort
  end
