...
class TestFakerInternet < Test::Unit::TestCase
  ...
  def test_ip_v4_cidr
    assert @tester.ip_v4_cidr.match(/^([0-9]{1,3}\.){3}[0-9]{1,3}\/[0-9]{1,2}$/)
  ...
  def test_ip_v6_cidr
    assert @tester.ip_v6_cidr.match(/^([0-9,a-f]{1,4}:){7}[0-9,a-f]{1,4}\/[0-9]{1,3}$/)

    1000.times do
      assert @tester.ip_v6_cidr.split('/').last.to_i <= 128
    end
  end
