...
class TestFakerAvatar < Test::Unit::TestCase
  ...
  def test_avatar_with_supported_format
    assert_match @tester.image(slug: 'faker', size: '300x300', format: 'jpg'), %r{https://robohash\.org/faker\.jpg}
  end
  ...
  def test_avatar_with_set
    assert_match @tester.image(slug: 'faker', size: '300x300', format: 'jpg', set: 'set2'), %r{https://robohash\.org/faker\.jpg.*set=set2}
  end

  def test_avatar_with_bgset
    assert_match @tester.image(slug: 'faker', size: '300x300', format: 'jpg', set: 'set1', bgset: 'bg1'), %r{https://robohash\.org/faker\.jpg.*bgset=bg1}
  end
end
