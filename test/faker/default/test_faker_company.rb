...
class TestFakerCompany < Test::Unit::TestCase
  ...
  def test_ein
    assert_match @tester.ein, /\d\d-\d\d\d\d\d\d\d/
  end

  def test_duns_number
    assert_match @tester.duns_number, /\d\d-\d\d\d-\d\d\d\d/
  end

  def test_logo
    assert_match @tester.logo, %r{https://pigment.github.io/fake-logos/logos/medium/color/\d+\.png}
  end

  def test_buzzword
    assert_match @tester.buzzword, /\w+\.?/
  end
  ...
  def test_australian_business_number
    ...
    assert_match abn, /\d{11}/
    assert_predicate((checksum % 89), :zero?)
  end

  def test_profession
    assert_match @tester.profession, /[a-z ]+\.?/
  end
  ...
  def test_russian_tax_number_default
    assert_match @tester.russian_tax_number, /\d{10}/
  end

  def test_russian_tax_number_individual
    assert_match @tester.russian_tax_number(type: :individual), /\d{12}/
  end

  def test_russian_tax_number_region
    assert_match @tester.russian_tax_number(region: '77'), /^77/
  end
  ...
  def test_sic_code
    assert_match @tester.sic_code, /\d\d\d\d/
  end
