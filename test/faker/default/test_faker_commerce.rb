...
class TestFakerCommerce < Test::Unit::TestCase
  ...
  def test_color
    assert_match @tester.color, /[a-z]+\.?/
  end

  def test_promotion_code
    assert_match @tester.promotion_code, /[A-Z][a-z]+[A-Z][a-z]+\d{6}/
  end

  def test_promotion_code_should_have_specified_number_of_digits
    assert_match @tester.promotion_code(digits: 3), /[A-Z][a-z]+[A-Z][a-z]+\d{3}/
  end

  def test_department
    assert_match @tester.department, /[A-Z][a-z]+\.?/
  end
  ...
  def test_product_name
    assert_match @tester.product_name, /[A-Z][a-z]+\.?/
  end

  def test_material
    assert_match @tester.material, /[A-Z][a-z]+\.?/
  end
  ...
  def test_brand
    assert_match @tester.brand, /[A-Z][a-z]+\.?/
  end

  def test_vendor
    assert_match @tester.vendor, /[A-Z][a-z]+\.?/
  end
end
