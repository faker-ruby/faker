...
class TestPtBrLocale < Test::Unit::TestCase
  ...
  def test_pt_br_address_methods
    ...
    assert_match Faker::Address.state_abbr, /^[A-Z]{2}$/

    assert Faker::Address.country.is_a? String

    assert Faker::Address.building_number.is_a? String
    assert_match Faker::Address.building_number, /^([0-9]+)|(s\/n)$/

    assert Faker::Address.street_suffix.is_a? String

    assert Faker::Address.secondary_address.is_a? String
    assert_match Faker::Address.secondary_address, /^[a-zA-Z.]+\s[0-9]+$/

    assert Faker::Address.postcode.is_a? String
    assert_match Faker::Address.postcode, /^[0-9]{5}-[0-9]{3}$/
  ...
  def test_pt_br_color_methods
    ...
    assert_match Faker::Color.hex_color, /^#([A-Fa-f0-9]{6}|[A-Fa-f0-9]{3})$/
  ...
  def test_pt_br_internet_methods
    ...
    assert_match Faker::Internet.free_email, /^[a-z0-9._\-]+@[a-z0-9]+.[a-z]+.([a-z]+)?$/i
  ...
  def test_pt_br_name_methods
    ...
    assert_match Faker::Name.initials, /[A-Z]{3}/
    assert_match Faker::Name.initials(number: 2), /[A-Z]{2}/
  end
  ...
  def test_pt_br_vehicle_methods
    ...
    assert_match Faker::Vehicle.license_plate, /(^[A-Z]{3}-[0-9]{4})|([A-Z]{3}[0-9]{1}[A-Z][0-9]{2})|([A-Z]{3}[0-9]{2}[A-Z][0-9])/

    assert Faker::Vehicle.license_plate(state_abbreviation: 'RJ').is_a? String
    assert_match Faker::Vehicle.license_plate(state_abbreviation: 'RJ'), /^[A-Z]{3}-[0-9]{4}/
  end
