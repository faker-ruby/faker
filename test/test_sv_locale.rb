...
class TestSVLocale < Test::Unit::TestCase
  ...
  def test_address_methods
    assert_match Faker::Address.postcode, /^\d{5}$/
  ...
  def test_sv_phone_number_methods
    assert_match Faker::PhoneNumber.cell_phone, /^07[036]{1}[\-\s]?\d{3}[\-\s]?\d{4}$/
    assert_match Faker::PhoneNumber.phone_number, /^\d{4}[\s\-]?\d{4,6}$/
  end
