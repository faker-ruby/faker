...
module Faker
  class Internet < Base
    ...
    class << self
      ...
      def email(_legacy_name = NOT_GIVEN, _legacy_separators = NOT_GIVEN, name: nil, separators: nil, domain: nil)
      ...
      def free_email(_legacy_name = NOT_GIVEN, name: nil)
        construct_email(
      ...
      def safe_email(_legacy_name = NOT_GIVEN, name: nil)
        construct_email(
          sanitize_email_local_part(username(specifier: name)),
      ...
      def username(_legacy_specifier = NOT_GIVEN, _legacy_separators = NOT_GIVEN, specifier: nil, separators: %w[. _])
        with_locale(:en) do
      ...
      #
      # @faker.version 2.1.3
      def password(_legacy_min_length = NOT_GIVEN, _legacy_max_length = NOT_GIVEN, _legacy_mix_case = NOT_GIVEN, _legacy_special_characters = NOT_GIVEN, min_length: 8, max_length: 16, mix_case: true, special_characters: false)
      ...
      def domain_name(_legacy_subdomain = NOT_GIVEN, subdomain: false, domain: nil)
        with_locale(:en) do
          if domain
            domain
              .split('.')
      ...
      def fix_umlauts(_legacy_string = NOT_GIVEN, string: '')
      ...
      def mac_address(_legacy_prefix = NOT_GIVEN, prefix: '')
      ...
      def url(_legacy_host = NOT_GIVEN, _legacy_path = NOT_GIVEN, _legacy_scheme = NOT_GIVEN, host: domain_name, path: "/#{username}", scheme: 'http')
        "#{scheme}://#{host}#{path}"
      end
      # rubocop:enable Metrics/ParameterLists
      ...
      def slug(_legacy_words = NOT_GIVEN, _legacy_glue = NOT_GIVEN, words: nil, glue: nil)
        glue ||= sample(%w[- _])
      ...
      def user_agent(_legacy_vendor = NOT_GIVEN, vendor: nil)
