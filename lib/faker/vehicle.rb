module Faker
  class Vehicle < Base
    ...
    class << self
      #ISO 3779
      def vin
        _, wmi, wmi_ext = fetch_all('vehicle.manufacture').sample(random: Faker::Config.random)

        c = VIN_CHARS.split('').reject{ |n| n == '.'}
        vehicle_identification_number = wmi.split('').concat( Array.new(14) { c.sample(random: Faker::Config.random) } )
      ...
      def manufacture
        fetch_all('vehicle.manufacture').sample(random: Faker::Config.random).first
      end
