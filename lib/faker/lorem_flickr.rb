...
module Faker
  class LoremFlickr < Base
    class << self
      ...
      def image(size = '300x300', search_terms = [], match_all = false, randomize = true)
        @url = build_url(size, nil, search_terms, match_all, randomize)
      end

      def grayscale_image(size = '300x300', search_terms = ['all'], match_all = false, randomize = true)
        raise ArgumentError, 'Search terms must be specified for grayscale images' unless search_terms.any?

        @url = build_url(size, 'g', search_terms, match_all, randomize)
      end

      def pixelated_image(size = '300x300', search_terms = ['all'], match_all = false, randomize = true)
        raise ArgumentError, 'Search terms must be specified for pixelated images' unless search_terms.any?

        @url = build_url(size, 'p', search_terms, match_all, randomize)
      end

      def colorized_image(size = '300x300', color = 'red', search_terms = ['all'], match_all = false, randomize = true)
        ...
        @url = build_url(size, color, search_terms, match_all, randomize)
      end
